...
def readS2fromFile(
    ...
    print(
        "Selected pixel region: tmxmin=%d, tmymin=%d, tmxmax=%d, tmymax=%d:"
        % (tmxmin, tmymin, tmxmax, tmymax)
    )
    ...
    def save_band(data, name, percentile_data=None):
        ...
        imageio.imsave(
            save_prefix + name + ".png", band_data
    ...
    if test_data:
        ...
        if run_60:
            np.save(
                out_per_image + "no_tiling/" + "data60_gt", data60_gt.astype(np.float32)
            )
            np.save(
            ...
            np.save(
                out_per_image + "no_tiling/" + "data20_gt", data20_gt.astype(np.float32)
            )
        ...
        print("Writing files for testing to:{}".format(out_per_image))
        save_test_patches60(
            data10_gt, data20_gt, data60_gt, out_per_image, patchSize=384, border=12
        )
        ...
        if run_60:
            save_random_patches60(
                data60_gt, data10_lr, data20_lr, data60_lr, out_per_image
            )
        else:
